...
function adstm_updparam() {
    return 'https://alidropship.com/?theme_update=davinci2';
}
...
function adstm_download_language_pack( $wplang = false ) {
    ...
    $translation = adstm_get_available_translations( $wplang ); //todo причём тут ещё и запрос на наличие надо делать языкового файла
...
function adstm_theme_api_call( $def, $action, $args ) {
    ...
    if (is_wp_error($request)) {
        $res = new WP_Error('themes_api_failed',
            __('<p>An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a></p>', 'dav2'), $request->get_error_message());
    } else {
