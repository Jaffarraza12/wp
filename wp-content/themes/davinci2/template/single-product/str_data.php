...
if(is_array($product['skuAttr'])) {
    ...
    foreach ($product['skuAttr'] as $k => $attr) {
        ...
        $str_data['offers'][] = [
            "@type" => "Offer",
            "url" => $_SERVER['SCRIPT_URI'] . '?sku=' . $k,
            "priceCurrency" => $product['currency'],
            "price" => $attr['_salePrice'],
            "priceValidUntil" => date('Y-m-d',
                strToTime('today + 30 days')
            ),
            "name" => $itemmeta['name'],
            "availability" => $attr['isActivity'] ? "https://schema.org/InStock" : "https://schema.org/OutStock",
    ...
    $str_data['offers'] = [
        ...
        "priceCurrency" => $product['currency'],
        "price" => $product['_salePrice'],
        "name" => $itemmeta['name'],
        "availability" => $product['stock']>0 ? "https://schema.org/InStock" : "https://schema.org/OutStock",
        "itemCondition" => "https://schema.org/NewCondition"
    ];
}
